...
namespace FuneePlugin
{
	...
	static internal class CentipedeAIPatch
	{
		...
		static bool IsRayhitOnAnyUnfortunatePlayer(RaycastHit rayHit)
		{
			...
			{
				MyLogger.Debug("rayhit = " + rayHit.transform.name + " | player = " + player.transform.name);
		...
		static void UpdatePatch(ref CentipedeAI __instance, ref int ___currentBehaviourStateIndex, ref bool ___clingingToCeiling, ref bool ___clingingToPlayer, ref bool ___triggeredFall)
		{
			...
			{
				//Do the ingame code for jumping down, but with the radius of the spherecast 3 times larger
				Ray ray = new Ray(__instance.transform.position, Vector3.down);

				if (Physics.SphereCast(ray, 6f, out RaycastHit rayHit, 60f, StartOfRound.Instance.playersMask) && !___clingingToPlayer && !Physics.Linecast(rayHit.transform.position, __instance.transform.position, StartOfRound.Instance.collidersAndRoomMask, QueryTriggerInteraction.Ignore) && !___triggeredFall)
				{
